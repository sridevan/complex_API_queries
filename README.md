...
| **UniProt-anchored aggregation** (Example 3) | Grouping every PDB chain associated with a UniProt accession by conformational state and relating these to ligand-binding states | `uniprot/superposition` → `pdb/entry/summary` → `pdb/entry/molecules` → `pdb/bound_molecules` → `pdb/compound/summary` | `(pdb_id, auth_asym_id)` |
